...
interface DataTableProps<TData> {
  ...
  meta?: PaginationMeta;
  page?: number;
  /** Page size used for the “x–y of z” range display. */
  pageSize?: number;
  ...
  /** Row id to highlight as selected (e.g. the currently peeked trace). */
...
export function DataTable<TData>({
  ...
  return (
    <div className="flex h-full flex-col gap-2">
        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="bg-surface-inset hover:bg-surface-inset">
          ...
          <TableBody>
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row) => {
                const isSelected =
                  row.getIsSelected() || (selectedRowId != null && row.id === selectedRowId);
                return (
                  <TableRow
                    key={row.id}
      ...
      {onPageChange && (
        <Pagination meta={meta} page={page} pageSize={pageSize} onPageChange={onPageChange} />
      )}
    </div>
  );
}
