...
const RequestLogRow: Component<{ log: RequestLog }> = (props) => {
  ...
  return (
    <>
      ...
      >
        <td class="px-4 py-2.5 text-xs text-fg-tertiary">
          <span class="mr-1 inline-block align-middle">
            <Show when={expanded()} fallback={<ChevronRight class="h-3 w-3" size={12} />}>
              <ChevronDown class="h-3 w-3" size={12} />
            </Show>
          </span>
          {new Date(props.log.startTime).toLocaleString()}
        </td>
        <td class="px-4 py-2.5 font-mono text-xs text-fg-primary">{props.log.model || "—"}</td>
        <td class="hidden px-4 py-2.5 text-xs text-fg-secondary md:table-cell">
          {props.log.provider || "—"}
        </td>
        ...
        <td class={cn("px-4 py-2.5 text-xs font-medium capitalize", statusColor(props.log.status))}>
          {props.log.status}
        </td>
        <td class="hidden px-4 py-2.5 text-right text-xs text-fg-tertiary lg:table-cell">
        ...
        <tr class="border-b border-border/50 bg-surface-inset/50">
          <td colspan={8} class="px-8 py-3">
            <div class="grid gap-3 text-xs lg:grid-cols-2">
              <div>
                <p class="mb-1 font-medium text-fg-secondary">Metadata</p>
                <pre class="max-h-40 overflow-auto rounded-md border border-border bg-surface-raised p-2 font-mono text-[11px] text-fg-tertiary">
                  {JSON.stringify(props.log.metadata, null, 2)}
                </pre>
              </div>
              <div>
                <p class="mb-1 font-medium text-fg-secondary">Messages</p>
                <pre class="max-h-40 overflow-auto rounded-md border border-border bg-surface-raised p-2 font-mono text-[11px] text-fg-tertiary">
                  {props.log.messages
          ...
          fallback={
            <div class="space-y-4">
              <div class="flex flex-wrap items-center gap-3">
                ...
                />
                <select
                  class="h-9 rounded-md border border-border bg-surface-raised px-3 text-[13px] text-fg-secondary"
                  value={status()}
                  onChange={(e) => {
                    ...
                    <table class="w-full text-sm">
                      <thead>
                        <tr class="border-b border-border text-left text-[11px] font-medium uppercase tracking-[0.06em] text-fg-tertiary">
                          ...
                          <th class="hidden px-4 py-2.5 text-right lg:table-cell">Duration</th>
                          <th class="hidden px-4 py-2.5 text-right lg:table-cell">TTFT</th>
                        </tr>
                      </thead>
                      <tbody>
                        {logs().map((log) => (
                        ...
                        {logs().map((log) => (
                          <tr class="border-b border-border/50 hover:bg-surface-overlay/40">
                            ...
                            <td class="px-4 py-2.5">
                              <span class="rounded bg-danger-subtle px-1.5 py-0.5 font-mono text-[10px] text-danger">
                                {log.exceptionType}
                              </span>
                            </td>
                            <td class="max-w-[300px] truncate px-4 py-2.5 text-xs text-fg-secondary">
...
const LogsContent: Component = () => (
  <div class="p-6">
    <Tabs defaultValue="requests">
      ...
      <TabsContent value="errors" class="mt-4">
        <ErrorLogsTab />
      </TabsContent>
    </Tabs>
  </div>
