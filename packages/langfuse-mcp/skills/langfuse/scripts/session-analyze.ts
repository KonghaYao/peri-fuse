...
const analyses = traces.map((trace: any, index: number) => {
  ...
  return {
    ...
    generations: items.filter((item) => item.type === "GENERATION"),
...
if (csvMode) {
  console.log(
    "traceIdx,traceId,timestamp,genIdx,model,input,cacheRead,cacheCreate,effective,output,latency,traceLatencySource,traceFlags,errorCategories",
  );
    ...
    for (const [index, generation] of analysis.generations.entries()) {
      ...
      console.log(
        [
          ...
          analysis.timestamp,
          index + 1,
          generation.model || generation.modelName || "?",
          tokens.input,
          tokens.cacheRead,
          tokens.cacheCreate,
          tokens.input,
          tokens.output,
          latency ?? "",
          analysis.metrics.latency.source,
          analysis.flags.map((flag) => flag.type).join(";"),
          analysis.errors.categories.join(";"),
        ].join(","),
      );
    }
  process.exit(0);
...
const aggregate = analyses.reduce(
  (sum, analysis) => {
    ...
    for (const category of analysis.errors.categories)
...
for (const analysis of analyses)
  console.log(
    `| ${analysis.idx} | ${isoToLocal(analysis.timestamp)} | ${analysis.name.replace(/\|/g, "\\|").slice(0, 50)} | ${fmt(analysis.metrics.effectiveNewTokens)} | ${analysis.metrics.llmCalls} | ${analysis.metrics.toolCalls} | ${fmtLatency(analysis.metrics.latency)} | ${analysis.errors.categories.join(",") || "-"} | ${analysis.flags.map((flag) => flag.type).join(",") || "-"} |`,
  );
...
if (detailMode)
  for (const analysis of analyses) {
    ...
    for (const [index, generation] of analysis.generations.entries()) {
      ...
      const latency = summarizeObservationLatency(generation);
      console.log(
        `| ${index + 1} | ${fmt(tokens.input)} | ${fmt(tokens.cacheRead)} | ${fmt(tokens.input)} | ${fmt(tokens.output)} | ${latency === null ? "N/A" : fmtLatency({ seconds: latency, source: "observations" })} |`,
      );
    }
  }

const diagnostics = analyses.flatMap((analysis) =>
  analysis.flags.map((flag) => ({ ...flag, trace: analysis.id })),
);
if (diagnostics.length) {
  console.log(`\n### Diagnostics (${diagnostics.length})\n`);
  for (const diagnostic of diagnostics)
    console.log(`- [${diagnostic.trace.slice(0, 8)}] ${diagnostic.description}`);
}
