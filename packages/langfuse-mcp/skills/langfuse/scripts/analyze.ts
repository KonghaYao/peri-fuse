 ...
 *
 * 用法:
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/analyze.ts [数量]           # 最近 N 条 trace 综合报告
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/analyze.ts --trace-id <id> # 单条 trace 详细报告
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/analyze.ts --tools [数量]   # 工具调用专项分析
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/analyze.ts --growth [数量]  # 上下文涨幅分析
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/analyze.ts --report [数量]  # 完整分析报告(全部维度)
 *
...
import { analyzeTrace, fetchAllObservations, type TraceAnalysis } from "./trace-analysis.ts";
...
function sectionOverview(traces: TraceAnalysis[]) {
  ...
  for (const t of traces) {
    aggIn += t.totalInput;
    aggOut += t.totalOutput;
    aggCache += t.totalCache;
    aggCacheCreation += t.totalCacheCreation;
    aggLLM += t.llmCalls;
    aggTool += t.toolCalls;
  }
  ...
  console.log(
    `  Cache read:     ${FMT(aggCache)} tokens (${PCT(aggCache, totalInputTraffic({ input: aggIn, cacheRead: aggCache, cacheCreate: aggCacheCreation }))} of input traffic)`,
  );
...
function sectionToolAnalysis(traces: TraceAnalysis[]) {
  ...
  const toolMap = new Map<
    string,
    { count: number; totalLatency: number; latencySamples: number; errors: number }
  >();
  for (const t of traces) {
  ...
  const tools = [...toolMap.entries()]
    .map(([name, stats]) => ({
      name,
      ...stats,
      avgLatency: stats.latencySamples ? stats.totalLatency / stats.latencySamples : null,
    }))
...
function sectionContextGrowth(traces: TraceAnalysis[]) {
  ...
  for (const t of traces) {
    ...
    console.log(
      `  Start: ${FMT(firstInput)} \u2192 End: ${FMT(lastInput)} (growth: ${growthPct}%)\n`,
    );
    ...
    for (let i = 0; i < g.length; i++) {
      ...
      const cacheCreateCells = Math.min(
        Math.round(cacheCreateWidth),
        barWidth - rawCells - cacheReadCells,
      );
      const usedWidth = rawCells + cacheReadCells + cacheCreateCells;
      const bar =
        "█".repeat(rawCells) +
        "░".repeat(cacheReadCells) +
        "▒".repeat(cacheCreateCells) +
        " ".repeat(barWidth - usedWidth);
      console.log(
        `  ${String(i + 1).padStart(2)} |${bar}| raw=${FMT(tokens.input)} cache-read=${FMT(tokens.cacheRead)} cache-create=${FMT(tokens.cacheCreation)}`,
      );
    }
  ...
  for (const [sid, s] of sessionMap) {
    ...
    console.log(
      `| ${label} | ${s.traces} | ${FMT(s.totalInput)} | ${FMT(s.totalOutput)} | ${FMT(avg)} |`,
    );
  }
...
function sectionSystemPrompt(_traces: TraceAnalysis[]) {
  ...
  console.log(
    "为避免输出或派生 prompt 内容，此脚本不分析 prompt 组成；请使用仅输出计数的 trace-messages.ts。\n",
  );
}
...
function sectionSummary(traces: TraceAnalysis[]) {
  ...
  for (const t of traces) {
    ...
    if (t.llmCalls > 10) flags.push(`LLM 调用较多（${t.llmCalls}）：${traceLabel(t)}`);
    const slowGen = t.genDetails.find((g) => g.latency !== null && g.latency > 60);
    if (slowGen) flags.push(`LLM 调用较慢（${slowGen.latency!.toFixed(0)}s）：${traceLabel(t)}`);
...
async function run(
  ...
  filters?: ReturnType<typeof parseFilterArgs>,
) {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--limit": {
        ...
        if (!Number.isNaN(n) && n > 0) limit = clampTraceLimit(n);
        break;
      }
      case "--days":
      case "--from":
