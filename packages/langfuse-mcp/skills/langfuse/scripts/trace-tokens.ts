 ...
 *
 * 用法:
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/trace-tokens.ts <traceId>
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/trace-tokens.ts --index <N> [--from/--to/--days/--tag/--user/--session/--name/--limit]
 */
import {
  fetchObservations,
  fetchTracesFiltered,
...
if (!traceId && index !== undefined) {
  ...
  if (index > traces.length) {
    console.log(`Index ${index} out of range (max ${traces.length}).`);
    process.exit(1);
  }
...
if (!traceId) {
  console.error("Usage: bun trace-tokens.ts <traceId>  or  --index <N>");
  process.exit(1);
}
...
console.log(
  `   Latency: ${fmtLatency(summarizeLatency(observations))} | Generations: ${generations.length}\n`,
);
...
for (let i = 0; i < roundTokens.length; i++) {
  ...
  if (i > 0) {
    ...
    if (curPct < prevPct - 10) {
      console.log(
        `  ⚠️ Round ${i + 1}: Cache hit dropped ${prevPct.toFixed(1)}% → ${curPct.toFixed(1)}% (${(prevPct - curPct).toFixed(0)}pp)`,
      );
      anomalies++;
    }
  }
  ...
  if (r.cacheCreate > 0) {
    console.log(
      `  📦 Round ${i + 1}: Cache creation = ${fmt(r.cacheCreate)} tokens (new prefix cached)`,
    );
    anomalies++;
  }
  ...
  if (r.effective > 5000) {
    console.log(
      `  🔴 Round ${i + 1}: Effective new = ${fmt(r.effective)} (>5K — check tool results or context injection)`,
    );
    anomalies++;
  }
...
for (let i = 1; i < roundTokens.length; i++) {
  ...
  if (currTraffic < prevTraffic * 0.85) {
    console.log(
      `  ✂️ Round ${i + 1}: Input traffic dropped ${fmt(prevTraffic)} → ${fmt(currTraffic)} (possible compact/truncation)`,
    );
    anomalies++;
  }
}
