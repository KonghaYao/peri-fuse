...
for (const observation of observations) {
  if (typeof observation?.id === "string" && !byId.has(observation.id))
...
for (const remainingId of sortIds([...byId.keys()].filter((id) => !visited.has(id))))
  printTree(remainingId, 0);
...
for (const item of audit.missingParents)
  console.log(`- MISSING_PARENT ${item.id} -> ${item.parentObservationId}`);
for (const cycle of audit.cycles) console.log(`- CYCLE ${cycle.join(" -> ")}`);

if (audit.duplicateIds.length || audit.missingParents.length || audit.cycles.length)
  process.exit(1);
