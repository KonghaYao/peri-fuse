...
if (!traceId && selectedIndex !== undefined) {
  ...
  if (selectedIndex > traces.length) {
    console.log(`Index ${selectedIndex} out of range (max ${traces.length}).`);
    process.exit(1);
  }
  traceId = traces[selectedIndex - 1].id;
}
if (!traceId) {
  console.error("Usage: bun trace-messages.ts <traceId>  or  --index <N>");
  process.exit(1);
}
...
function diffMessages(previous: MessageSummary[], current: MessageSummary[]) {
  ...
  for (let index = 0; index < current.length; index++)
    if (!previous[index] || previous[index].signature !== current[index].signature) changed++;
  return { added: Math.max(0, current.length - previous.length), changed };
}
function systemSignature(messages: MessageSummary[]) {
  return messages
...
console.log(
  `## Trace: ${trace?.name ? String(trace.name).slice(0, 60) : String(trace?.id || traceId).slice(0, 12)}`,
);
...
for (const [index, generation] of generations.entries()) {
  ...
  for (const [messageIndex, message] of messages.entries()) {
    roles[message.role === "tool" ? "tool" : message.role] =
      (roles[message.role === "tool" ? "tool" : message.role] || 0) + 1;
    if (!previous[messageIndex] || previous[messageIndex].signature !== message.signature)
      changedChars += message.chars;
  }
  console.log(
    `| ${index + 1} | ${roles.system} | ${roles.user} | ${roles.assistant} | ${roles.tool} | ${messages.length} | ${diff.added + diff.changed} | ${fmt(changedChars)} |`,
  );
  previous = messages;
}
...
const generationIndex = new Map(
  generations.map((generation: any, index: number) => [generation, index + 1]),
);
for (const [index, segment] of splitGenerationAgentSegments(observations, generations).entries()) {
  ...
  if (segment.agentObservationId === null || segment.agentLabel === "unknown") {
    console.log(
      `| ${index + 1} | unknown | ${roundNumbers.join(",")} | - | - | Unknown ownership; not compared across agents |`,
    );
    continue;
  }
  const signatures = segment.generations.map((generation) =>
    systemSignature(messagesByGeneration.get(generation) || []),
  );
  const baseline = signatures[0] || "";
  const changedRounds = signatures.flatMap((signature, round) =>
    signature !== baseline ? [roundNumbers[round]] : [],
  );
  const count = (messagesByGeneration.get(segment.generations[0]) || []).filter(
  ...
  console.log(
    `| ${index + 1} | ${label} | ${roundNumbers.join(",")} | ${count} | ${changedRounds.length} | ${status} |`,
  );
}
