/** Pure telemetry calculations; kept separate from API access and CLI parsing. */
import { fmt, ms } from "./format.ts";

export function genTokens(generation: any) {
  const usageDetails =
    generation?.usageDetails && typeof generation.usageDetails === "object"
      ? generation.usageDetails
      : {};
  const legacyUsage =
    generation?.usage && typeof generation.usage === "object" ? generation.usage : {};
...
export function summarizeTokens(generations: any[]): TokenSummary {
  const total = generations.reduce(
    (sum, generation) => {
      ...
      sum.cacheCreate += tokens.cacheCreate;
      return sum;
    },
...
export function summarizeLatency(observations: any[]): LatencySummary {
  const agentRun = observations.find(
    (observation) => observation?.type === "AGENT" && observation?.name === "agent-run",
  );
  ...
  return {
    seconds:
      (Math.max(...intervals.map((value) => value.end)) -
        Math.min(...intervals.map((value) => value.start))) /
      1000,
    source: "observations",
  };
}
...
export function fmtLatency(latency: LatencySummary) {
  return latency.seconds === null ? "N/A" : ms(latency.seconds);
}
...
export interface TraceMetrics {
  ...
  effectiveNewTokens: number;
  latency: LatencySummary;
}
export function summarizeTraceMetrics(observations: any[]): TraceMetrics {
  const tokens = summarizeTokens(
    observations.filter((observation) => observation?.type === "GENERATION"),
  );
  return {
    llmCalls: tokens.calls,
    toolCalls: observations.filter((observation) => observation?.type === "TOOL").length,
    inputTokens: tokens.input,
    outputTokens: tokens.output,
    cacheReadTokens: tokens.cacheRead,
    cacheCreateTokens: tokens.cacheCreate,
    effectiveNewTokens: tokens.effective,
...
export function estimateCost(
  model: string,
  tokens: Pick<TokenSummary, "input" | "output" | "cacheRead" | "cacheCreate">,
): number {
  const key = Object.keys(MODEL_PRICES).find((candidate) =>
...
export type ErrorCategory =
  | "provider_or_stream_failure"
  | "tool_failure"
  | "cancelled_or_user_aborted"
  | "timeout"
  | "rate_limit"
  | "max_iterations"
  | "unknown_failure";
...
export function summarizeErrors(observations: any[]): ErrorSummary {
  ...
  for (const observation of observations) {
    ...
    const errorClass = String(
      observation?.metadata?.error_class || observation?.output?.error_class || "",
    ).toLowerCase();
    const category = ERROR_CLASS_CATEGORIES[errorClass];
    const isError =
      String(observation?.level || "").toUpperCase() === "ERROR" ||
      Boolean(category) ||
      ["ERROR", "FAILED", "CANCELLED", "ABORTED", "INTERRUPTED"].includes(status);
    if (!isError) continue;
    if (["CANCELLED", "ABORTED", "INTERRUPTED"].includes(status))
      categories.add("cancelled_or_user_aborted");
    else if (category) {
      categories.add(category);
      if (category === "provider_or_stream_failure" && observation?.type === "GENERATION")
        failedLlmCalls++;
      if (category === "tool_failure" && observation?.type === "TOOL") failedToolCalls++;
    } else if (observation?.type === "TOOL") {
      categories.add("tool_failure");
      failedToolCalls++;
    } else if (observation?.type === "GENERATION") {
      categories.add("provider_or_stream_failure");
      failedLlmCalls++;
    } else categories.add("unknown_failure");
  }
  return {
    hasError: categories.size > 0,
...
export function detectAnomalies(metrics: TraceMetrics, _model?: string): Anomaly[] {
  ...
  const inputTraffic = totalInputTraffic({
    input: metrics.inputTokens,
    cacheRead: metrics.cacheReadTokens,
    cacheCreate: metrics.cacheCreateTokens,
  });
  if (metrics.effectiveNewTokens > 20_000)
    anomalies.push({
      type: "high_effective",
      severity: "high",
      description: `有效新 token = ${fmt(metrics.effectiveNewTokens)} (>20K)，上下文可能持续膨胀`,
    });
  if (inputTraffic > 5000 && metrics.cacheReadTokens / inputTraffic < 0.1)
    anomalies.push({
      type: "cache_drop",
      severity: "medium",
      description: `缓存读取占输入流量仅 ${pct(metrics.cacheReadTokens, inputTraffic)}，可能是新 session 或 prompt 变更`,
    });
  if (metrics.inputTokens > 100_000 && metrics.outputTokens / metrics.inputTokens < 0.001)
    anomalies.push({
      type: "tiny_output",
      severity: "medium",
      description: `输出/输入比极低 ${pct(metrics.outputTokens, metrics.inputTokens)}，大量上下文可能无用`,
    });
  if (metrics.latency.seconds !== null && metrics.latency.seconds > 120)
    anomalies.push({
      type: "high_latency",
      severity: "low",
      description: `真实耗时 ${fmtLatency(metrics.latency)} (>2min)，单个 trace 耗时较长`,
    });
  if (metrics.llmCalls > 10 || metrics.toolCalls > 10) {
    const high =
      metrics.effectiveNewTokens > 20_000 ||
      (metrics.latency.seconds !== null && metrics.latency.seconds > 120);
    anomalies.push({
      type: "loop",
      severity: high ? "high" : "medium",
      description: `长 loop：LLM=${metrics.llmCalls}，Tools=${metrics.toolCalls}，有效新增=${fmt(metrics.effectiveNewTokens)}，真实耗时=${fmtLatency(metrics.latency)}`,
    });
  }
  return anomalies;
}
...
export interface AgentSegment {
  agentObservationId: string | null;
  agentLabel: string;
...
export function splitGenerationAgentSegments(
  observations: any[],
  generations?: any[],
): AgentSegment[] {
  const byId = new Map(
  ...
  const ordered = (
    generations || observations.filter((observation) => observation?.type === "GENERATION")
  )
    .map((generation, index) => ({ generation, index }))
    .sort(
      (a, b) => observationTime(a.generation, a.index) - observationTime(b.generation, b.index),
    );
    ...
    while (current?.parentObservationId && !visited.has(current.parentObservationId)) {
      ...
      if (parent.type === "AGENT")
        return {
          ...
          label:
            parent.name === "agent-run" || String(parent.name || "").startsWith("subagent-")
              ? parent.name
              : "unknown",
        };
      current = parent;
  ...
  for (const { generation } of ordered) {
    ...
    if (!previous || previous.agentObservationId !== identity.id)
      segments.push({
        agentObservationId: identity.id,
