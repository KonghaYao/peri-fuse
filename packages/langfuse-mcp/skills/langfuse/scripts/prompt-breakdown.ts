 ...
 *
 * 用法:
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/prompt-breakdown.ts <traceId>
 *   bun packages/langfuse-mcp/skills/langfuse/scripts/prompt-breakdown.ts --index <N> [过滤选项]
 */
import {
  fetchObservations,
  fetchTracesFiltered,
...
if (!traceId) {
  console.error("Usage: bun prompt-breakdown.ts <traceId>  or  --index <N>");
  process.exit(1);
}
...
if (sysSections.length > 0) {
  ...
  console.log("|-----------------|-------|-------------|------------|");
  console.log(
    `| ${sysSections.length} | ${fmt(sysTotal)} | 100.0% | ${pct(sysTotal, totalChars)} |`,
  );
}
