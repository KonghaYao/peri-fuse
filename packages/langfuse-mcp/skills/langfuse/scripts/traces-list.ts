 ...
 *
 * 用法: bun packages/langfuse-mcp/skills/langfuse/scripts/traces-list.ts [N] [选项]
 *
 * 过滤:
 *   --from <ISO>   起始时间
 *   --to <ISO>     结束时间
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    "用法: bun traces-list.ts [N] [--from ISO] [--to ISO] [--days N] [--tag tag] [--user id] [--session id] [--name text] [--limit N]",
  );
  process.exit(0);
}
...
console.log(
  `  Raw input: ${fmt(agg.input)}  Output: ${fmt(agg.output)}  Cache read: ${fmt(agg.cache)} (${pct(agg.cache, totalInputTraffic({ input: agg.input, cacheRead: agg.cache, cacheCreate: agg.cacheCreate }))} of input traffic)  Cache create: ${fmt(agg.cacheCreate)}`,
);
console.log(`  Effective new: ${fmt(agg.input)}`);
console.log(
  `  Output/Input: ${agg.input > 0 ? `${((agg.output / agg.input) * 100).toFixed(2)}%` : "-"}`,
);
