...
import {
  ...
  genTokens,
  parseFilterArgs,
  splitGenerationAgentSegments,
  summarizeErrors,
  summarizeLatency,
  summarizeTokens,
  summarizeTraceMetrics,
  totalInputTraffic,
} from "../skills/langfuse/scripts/lib.ts";
...
describe("Langfuse report pure logic", () => {
  test("usageDetails 将 raw input、cache read、cache creation、output 各计一次且不为负", () => {
    ...
    expect(genTokens(generation)).toEqual({
      input: 100,
      cacheRead: 20,
      cacheCreate: 30,
      output: 40,
    ...
    ).toEqual({ input: 100, cacheRead: 20, cacheCreate: 30, output: 40, effective: 100, calls: 2 });
    expect(
      estimateCost("unknown-model", { input: 100, cacheRead: 20, cacheCreate: 30, output: 40 }),
    ).toBeCloseTo((100 * 2 + 20 * 0.5 + 30 * 2 + 40 * 10) / 1_000_000);
  });
  ...
  test("usageDetails 按字段回退 legacy usage 与常见别名", () => {
    expect(
      genTokens({
        usageDetails: {},
        usage: { prompt_tokens: 11, completion_tokens: 12, cache_read: 13, cache_creation: 14 },
      }),
    ).toEqual({
      input: 11,
      output: 12,
  ...
  test("raw input 小于 cache read 时 effective 不为负且比例分母包含全部输入桶", () => {
    const summary = summarizeTokens([
      { usageDetails: { input: 5, cache_read_input_tokens: 100, cache_creation_input_tokens: 10 } },
    ]);
      ...
      summarizeLatency([
        {
          ...
          endTime: "2026-01-01T00:00:12.500Z",
        },
      ]),
    ).toEqual({ seconds: 12.5, source: "agent-run" });
  });

  test("缺少 agent-run 时使用 observation 时间包络，完全缺失时不可用", () => {
    expect(
      summarizeLatency([
        { startTime: "2026-01-01T00:00:03.000Z", endTime: "2026-01-01T00:00:05.000Z" },
        { startTime: "2026-01-01T00:00:01.000Z", endTime: "2026-01-01T00:00:08.000Z" },
      ]),
    ).toEqual({ seconds: 7, source: "observations" });
    expect(summarizeLatency([{ startTime: "invalid" }])).toEqual({
      seconds: null,
      source: "unavailable",
    });
    ...
    const observations = [
      {
        ...
        name: "agent-run",
        startTime: "2026-01-01T00:00:00.000Z",
        endTime: "2026-01-01T00:02:10.000Z",
      },
      ...Array.from({ length: 11 }, () => ({
        type: "GENERATION",
        usageDetails: { input: 3000, cache_read_input_tokens: 1000 },
      })),
      ...Array.from({ length: 12 }, () => ({ type: "TOOL" })),
    ];
    const loop = detectAnomalies(summarizeTraceMetrics(observations)).find(
      (item) => item.type === "loop",
    );
  ...
  test("错误分类不会保留 prompt、工具结果或错误正文", () => {
    ...
    expect(summary).toEqual({
      hasError: true,
      categories: ["cancelled_or_user_aborted", "provider_or_stream_failure", "tool_failure"],
      failedLlmCalls: 1,
      failedToolCalls: 1,
    });
  ...
  test("稳定 error_class 映射为报告分类，不退化为 unknown", () => {
    ...
    expect(summary).toEqual({
      hasError: true,
      categories: ["provider_or_stream_failure", "rate_limit", "timeout", "tool_failure"],
      failedLlmCalls: 0,
      failedToolCalls: 0,
    });
  });
  ...
  test("主 agent、subagent、主 agent 被拆为连续的独立段", () => {
    const observations = [
      { id: "main", type: "AGENT", name: "agent-run" },
      { id: "child", type: "AGENT", name: "subagent-review" },
      {
        id: "g1",
        type: "GENERATION",
        parentObservationId: "main",
    ...
    expect(
      splitGenerationAgentSegments(observations).map((segment) => [
        segment.agentObservationId,
        segment.generationIds,
      ]),
    ).toEqual([
      ["main", ["g1"]],
      ["child", ["g2"]],
