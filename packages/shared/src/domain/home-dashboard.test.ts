...
describe("LANGFUSE_HOME_DASHBOARD", () => {
  it("parses through DashboardDomainSchema the way the worker upsert consumes it", () => {
    ...
    expect(parsed.definition.widgets.every((w) => w.type === "preset")).toBe(true);
  });

  it("places every registered preset exactly once", () => {
    const placedPresetIds = LANGFUSE_HOME_DASHBOARD.definition.widgets.flatMap((w) =>
      w.type === "preset" ? [w.presetId] : [],
    );

    expect([...placedPresetIds].sort()).toEqual([...HOME_DASHBOARD_PRESET_IDS].sort());
    // Placement ids must be unique within the dashboard (react keys,
    // scheduler ids, delete targets).
    const placementIds = LANGFUSE_HOME_DASHBOARD.definition.widgets.map((w) => w.id);
