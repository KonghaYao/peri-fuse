...
export function createSecureOutboundLookup(
  ...
  return (hostname, options, callback) => {
    lookup(hostname, options, (err, address, family) => {
      ...
      try {
        validateConnectionTimeLookupResult(hostname, address, validationOptions);
      } catch (error) {
...
function validateConnectionTimeLookupResult(
  ...
  const lookupAddresses = Array.isArray(address) ? address : [{ address, family: undefined }];

  for (const lookupAddress of lookupAddresses) {
    validateOutboundResolvedIp({
      hostname,
      ip: lookupAddress.address,
      whitelist: validationOptions.whitelist ?? DEFAULT_OUTBOUND_URL_VALIDATION_WHITELIST,
