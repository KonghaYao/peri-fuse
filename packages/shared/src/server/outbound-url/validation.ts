...
import { isHostnameBlocked, isIPAddress, isIPBlocked } from "../webhooks/ipBlocking";
...
export async function resolveHost(hostname: string): Promise<string[]> {
  ...
  if (!ips.size) {
    throw new OutboundUrlValidationError("dns-lookup-failed", `DNS lookup failed for ${hostname}`);
  }
  return [...ips];
}
...
export function parseOutboundUrl(urlString: string): URL {
  ...
  try {
    ...
    throw new OutboundUrlValidationError("invalid-syntax", "Invalid URL syntax");
  }
...
export async function validateOutboundUrlHost({
  ...
  if (isHostnameBlocked(hostname)) {
    throw new OutboundUrlValidationError("blocked-hostname", "Blocked hostname detected");
  }
...
export function validateOutboundResolvedIp({
  ...
  if (isIPBlocked(ip, whitelist.ips, whitelist.ip_ranges)) {
    ...
    throw new OutboundUrlValidationError("blocked-ip", "Blocked IP address detected");
  }
}
...
function assertValidUrlEncoding(urlString: string): void {
  ...
  try {
    ...
    throw new OutboundUrlValidationError("invalid-encoding", "Invalid URL encoding");
  }
}
