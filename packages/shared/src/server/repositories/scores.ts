...
import type { AppliedFilter } from "../queries/clickhouse-sql/clickhouse-filter";
import {
...
import {
  liteGetScoresForTraces,
  liteGetScoresTable,
  liteGetScoresTableCount,
...
export const upsertScore = async (score: Partial<ScoreRecordReadType>) => {
  ...
  await upsertClickhouse({
    ...
    tags: { projectId: score.project_id ?? "" },
  });
};
...
>(
  ...
  if (isLiteMode()) {
    ...
    return rows.map((row) => {
      ...
      if (includeHasMetadata) {
        Object.assign(score, {
          hasMetadata: Object.keys(row.metadata ?? {}).length > 0,
        });
      }
      return score;
    });
  ...
  return rows.map((row) => {
    ...
    recordDistribution("langfuse.query_by_id_age", Date.now() - score.timestamp.getTime(), {
      table: "scores",
    });
...
>(
  ...
  >({
    ...
    params: {
      ...
      limit: limit,
      offset: offset,
      ...(minTimestamp ? { minTimestamp: convertDateToClickhouseDateTime(minTimestamp) } : {}),
    },
    tags: { projectId },
    clickhouseConfigs,
  });
...
const uniqueNonEmptyStrings = (values: string[]) => [...new Set(values)].filter(Boolean);
...
export async function queryScoreRecordsForExperimentItems({
  ...
  const input = {
    params: {
      ...
      ...(toTimestamp ? { maxStartTime: convertDateToClickhouseDateTime(toTimestamp) } : {}),
      scoreLimit,
...
export async function queryScoreRecordsForExperiments({
  ...
  const input = {
    params: {
      ...
      ...(toTimestamp ? { startTimeTo: convertDateToClickhouseDateTime(toTimestamp) } : {}),
      scoreLimit,
      dataTypes: LISTABLE_SCORE_TYPES.map((type) => type.toString()),
    },
    tags: {
...
export const getScoresGroupedByNameSourceType = async ({
  ...
  scoresFilter.push(
    ...createFilterFromFilterState(filter, scoresColumnsTableUiColumnDefinitions, scoresTableCols),
  );

  // Separate scores-only filters from event/experiment filters
  const nonEventFilters = scoresFilter.filter((f) => !f.clickhouseTable.startsWith("events_"));
  ...
  const eventFilterState = filter.filter((filterEntry) =>
    scoresEventsFilterMapping.some((col) => matchesUiColumnMapping(col, filterEntry.column)),
  );

  let eventsCTE = "";
  const eventsCTEParams: Record<string, unknown> = {};
  const hasEventsFilters = eventFilterState.length > 0;

  let eventsFilterRes: AppliedFilter | undefined;
  if (hasEventsFilters) {
    ...
    const { query: cteQuery, params: cteParams } = eventsBuilder.buildWithParams();
  ...
  const rows = await queryClickhouse<{
    ...
    tags: { projectId },
    preferredClickhouseService: hasEventsFilters ? "EventsReadOnly" : "ReadOnly",
  });
...
export const getNumericScoresGroupedByName = async (projectId: string, filter?: FilterState) => {
...
export const getBooleanScoresGroupedByName = async (projectId: string, filter?: FilterState) => {
  const chFilter = filter
    ? createFilterFromFilterState(filter, scoresColumnsTableUiColumnDefinitions, scoresTableCols)
    : undefined;
...
export const getCategoricalScoresGroupedByName = async (
  ...
  return rows.map((row) => {
    ...
    if (configCategories && Array.isArray(configCategories)) {
      ...
      const mergedValues = Array.from(new Set([...row.values, ...allPossibleValues])).slice(
        0,
...
const getScoresUiGeneric = async <T>(props: {
  ...
  scoresFilter.push(
    ...createFilterFromFilterState(filter, scoresTableUiColumnDefinitions, scoresTableCols),
  );
  const scoresFilterRes = scoresFilter.apply();

  // Only join traces for rows or if there is a trace filter on counts
  const performTracesJoin =
    props.select === "rows" || scoresFilter.some((f) => f.clickhouseTable === "traces");
...
const getScoresUiGenericFromEvents = async <T>(props: {
  ...
  const scoreOnlyFilters = scoresFilter.filter((f) => f.clickhouseTable !== "traces");
  ...
  const traceFilterState = filter.filter((filterEntry) =>
    scoresTraceFilterEventsMapping.some((col) => matchesUiColumnMapping(col, filterEntry.column)),
  );

  const matchedOrderByColumn = orderBy
    ? findUiColumnMapping(scoresTableUiColumnDefinitionsFromEvents, orderBy.column)
    : null;
  const orderByColumn =
    matchedOrderByColumn?.clickhouseTableName === "traces" ? matchedOrderByColumn : null;
  ...
  if (needsTracesCTE) {
    ...
    const { query: cteQuery, params: cteParams } = tracesEventsBuilder.buildWithParams();
...
export const getScoreNames = async (projectId: string, timestampFilter: FilterState) => {
  const chFilter = new FilterList(
    createFilterFromFilterState(timestampFilter, scoresTableUiColumnDefinitions, scoresTableCols),
  );
...
const getDistinctScoreNamesStartTimeFrom = (
  ...
  return scoreTimestampFilter ? convertDateToClickhouseDateTime(scoreTimestampFilter.value) : null;
};
...
export const getScoresForBlobStorageExport = (
  projectId: string,
  minTimestamp: Date,
  maxTimestamp: Date,
) =>
...
export const getScoresForBlobStorageExportParquet = (
  projectId: string,
  minTimestamp: Date,
  maxTimestamp: Date,
) =>
...
export const getScoresForAnalyticsIntegrations = async function* (
  ...
  const selectedTraces =
    traceAttributesSource === "events"
      ? buildAnalyticsScoreTraceAttributesFromEventsCte(projectId, minTimestamp, maxTimestamp)
      : buildAnalyticsScoreTraceAttributesCte(projectId, minTimestamp, maxTimestamp);
...
export const getScoreMetadataById = async (
  ...
  return rows
    .map((row) => parseMetadataCHRecordToDomain(row.metadata as Record<string, string>))
    .shift();
};
...
export const _handleGenerateScoresForPublicApi = async ({
  ...
  if (isLiteMode()) {
    const records = pagination
      ? await liteGetScoresTable(projectId, pagination.limit, pagination.page - 1, scoresFilter)
    ...
    const traceIds = [
      ...new Set(records.map((r) => r.trace_id).filter((id): id is string => Boolean(id))),
    ];
    const traceInfo = await liteGetTraceInfoByIds(projectId, traceIds);

    return records.map((record) => {
      const domainScore = convertClickhouseScoreToDomain(record as ScoreRecordReadType);
      const info = record.trace_id ? traceInfo.get(record.trace_id) : undefined;
      return {
        ...domainScore,
        trace: {
...
const ANNOTATION_COLUMNS_V3 = ["s.author_user_id as author_user_id", "s.queue_id as queue_id"];
...
export function transformBooleanValueForFilter(v: "true" | "false"): number {
  if (v === "true") return 1;
  if (v === "false") return 0;
  throw new InternalServerError(`transformBooleanValueForFilter received unexpected value: ${v}`);
}
...
function buildDynamicFilters(params: ListFilterParams): {
      ...
      case ScoreDataTypeEnum.NUMERIC: {
        ...
        extraParams[varName] = params.value.map((v) => {
          ...
          if (!Number.isFinite(n)) {
            throw new InternalServerError(`NUMERIC value filter received non-finite value: ${v}`);
          }
          return n;
        });
        break;
  ...
  const allClauses = [compiled.query, ...extraClauses].filter(Boolean).join(" AND ");
...
export function polymorphicValueForV3(score: {
  ...
  switch (score.dataType) {
    ...
    default: {
      ...
      throw new InternalServerError(`Score has unknown dataType: ${_exhaustiveCheck as string}`);
    }
  }
}
...
function deriveSubjectForV3(
  ...
  if (!score.traceId) {
    throw new InternalServerError(`Score ${score.id} has kind=trace but missing traceId`);
  }
  return { kind: "trace", id: score.traceId };
}

export function scoreDomainToV3(score: ScoreDomain, fields: ScoreFieldGroupV3[]): APIScoreV3 {
  return {
    id: score.id,
    ...
    ...(fields.includes("annotation")
      ...
      : {}),
    ...(fields.includes("subject") ? { subject: deriveSubjectForV3(score) } : {}),
  } as APIScoreV3;
}
...
export async function listScoresV3ForPublicApi(
  ...
  const { query: filterClause, params: filterParams } = buildDynamicFilters(params);
  ...
  const input = {
    params: {
      ...
      ...(params.cursor && {
        lastTimestamp: convertDateToClickhouseDateTime(params.cursor.lastTimestamp),
  ...
  const records = await queryClickhouse<ScoreRecordReadType>({
    query: buildV3ListQuery(Boolean(params.cursor), params.fields, filterClause),
    params: input.params,
    tags: input.tags,
    preferredClickhouseService: "ReadOnly",
  });
  ...
  for (const row of pageRecords) {
    try {
      items.push(scoreDomainToV3(convertClickhouseScoreToDomain(row), params.fields));
    } catch (error) {
  ...
  return {
    data: filterAndValidateV3GetScoreList(items, (error) => {
      logger.error("v3 score row dropped from response: schema validation error", {
        issues: error.issues,
        projectId: params.projectId,
      });
    }),
