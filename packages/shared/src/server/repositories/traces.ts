...
import type { ColumnDefinition, UiColumnMappings } from "../../tableDefinitions";
...
export const checkTraceExistsAndGetTimestamp = async ({
  ...
  const observationFilter = tracesFilter.find((f) => f.clickhouseTable === "observations");
  ...
  const input = {
    params: {
      ...
      ...(maxTimeStamp ? { maxTimeStamp: convertDateToClickhouseDateTime(maxTimeStamp) } : {}),
      ...(exactTimestamp
  ...
  return {
    ...
    timestamp: rows.length > 0 ? parseClickhouseUTCDateTimeFormat(rows[0].timestamp) : undefined,
  };
};
...
export const upsertTrace = async (trace: Partial<TraceRecordReadType>) => {
  ...
  await upsertClickhouse({
    ...
    tags: { projectId: trace.project_id ?? "" },
  });
};
...
export const getTracesByIds = async (
  ...
  return records.map((record) => convertClickhouseToDomain(record, DEFAULT_RENDERING_PROPS));
};
...
export const getTracesBySessionId = async (
  ...
  traces.forEach((trace) => {
    recordDistribution("langfuse.traces_by_session_id_age", Date.now() - trace.timestamp.getTime());
  });
...
export const getTraceByIdFromTracesTable = async ({
  ...
  const res = records.map((record) => convertClickhouseToDomain(record, renderingProps));

  res.forEach((trace) => {
    recordDistribution("langfuse.query_by_id_age", Date.now() - trace.timestamp.getTime(), {
      table: "traces",
    });
  });
...
export const getTracesForBlobStorageExport = (
  projectId: string,
  minTimestamp: Date,
  maxTimestamp: Date,
) =>
...
export const getTracesForBlobStorageExportParquet = (
  projectId: string,
  minTimestamp: Date,
  maxTimestamp: Date,
) =>
...
async function buildTracesBaseQuery(
  ...
  if (select.includeObservations || select.includeMetrics || filtersNeedObservations) {
    const shouldUseFinal =
      (select.includeMetrics || filtersNeedObservations) && !disableObservationsFinal;
  ...
  const chOrderBy =
    (orderByToClickhouseSql(orderBy || [], traceOrderByColumns) || "ORDER BY t.timestamp desc") +
  ...
  const limitByClause = shouldUseSkipIndexes ? "LIMIT 1 by t.id, t.project_id" : "";
  ...
  if (select.count) {
    ...
    const ioDedup = shouldUseSkipIndexes ? "ORDER BY event_ts DESC LIMIT 1 BY id, project_id" : "";
    ctes.push(`io AS (
  ...
  const needsCteToTimeFilter =
    toTimeFilter && (propagateObservationsTimeBounds || (!select.count && select.includeIO));
...
export const generateTracesForPublicApi = async ({
  ...
  if (isLiteMode()) {
    const rows = await liteGetTracesTable({
      ...
      orderBy: orderBy ? { column: orderBy.column, order: orderBy.order } : undefined,
      filter,
    });
...
export const getTracesCountForPublicApi = async ({
  ...
  const needsComplexQuery = filter.some(
    (f) =>
      ...
      (f.clickhouseTable === "traces" &&
        !["user_id", "session_id", "metadata"].some((c) => f.field.includes(c))),
  );
