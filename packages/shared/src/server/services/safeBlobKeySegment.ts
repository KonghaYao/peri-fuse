...
export function safeBlobKeySegment(
  ...
  const hash = createHash("sha256").update(segment, "utf8").digest("hex").slice(0, HASH_HEX_LENGTH);
 ...
 */
export function safeBlobFilenameStem(stem: string, filenameSuffix: string): string {
