...
const resolve = (filterState: Parameters<typeof applyCommentFilters>[0]["filterState"]) =>
...
describe("applyCommentFilters", () => {
  ...
  it("still applies content when the count predicate is a no-op", async () => {
    ...
    await expect(resolve([count(">=", 0), content("needs-review")])).resolves.toEqual(
      selection(matchingIds),
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      countResults: [["matching-observation", "first-only"], ["matching-observation"]],
      contentResults: [],
    },
  ])("$name", async ({ filters, countResults, contentResults }) => {
    const countLookup = mockResults("getObjectIdsByCommentCount", ...countResults);
    const contentLookup = mockResults("getObjectIdsByCommentContent", ...contentResults);

    await expect(resolve(filters)).resolves.toEqual(selection(["matching-observation"]));
  ...
  it("validates the final content-filter intersection", async () => {
    ...
    await expect(resolve([content("broad"), content("narrow")])).resolves.toEqual(
      selection(matchingIds),
    );
  });
});
