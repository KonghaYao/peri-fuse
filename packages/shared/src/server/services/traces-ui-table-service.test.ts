...
describe("getTraceDeleteCursorPageFromTraces", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    let counter = 0;
    mockClickhouseCompliantRandomCharacters.mockImplementation(() => `x${++counter}`);
  ...
  it("uses canonical cursor ordering without FINAL", async () => {
    ...
    expect(normalizedQuery).toContain("ORDER BY t.timestamp DESC, t.id DESC, t.event_ts DESC");
