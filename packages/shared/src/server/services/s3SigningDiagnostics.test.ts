...
describe("summarizeS3Error", () => {
  it("extracts structured fields from an AWS SDK service exception", () => {
    const err = Object.assign(new Error("The request signature we calculated"), {
  ...
  it("extracts GCS-specific Details from the XML error body", () => {
    ...
    expect(summary.details).toBe("Multipart upload is not supported in Rapid storage class.");
  });
