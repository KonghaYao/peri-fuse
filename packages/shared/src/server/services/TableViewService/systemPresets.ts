...
export const isSystemTableViewPresetId = (id: string | undefined | null): boolean =>
  !!id?.startsWith(SYSTEM_TABLE_VIEW_PRESET_ID_PREFIX);
...
> = {
  [TableViewPresetTableName.ObservationsEvents]: OBSERVATIONS_EVENTS_SYSTEM_TABLE_VIEW_PRESETS,
};

export const getSystemTableViewPresets = (tableName: TableViewPresetTableName) =>
...
export const getSystemTableViewPresetByTableAndId = (
  ...
  getSystemTableViewPresets(tableName).find((preset) => preset.id === id) ?? null;

export const getSystemTableViewPresetById = (id: string): SystemTableViewPreset | null => {
