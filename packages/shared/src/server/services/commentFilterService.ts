...
type CommentContentFilter = Extract<z.infer<typeof singleFilter>, { type: "string" }>;
...
function filterRangeIncludesZero(
  ...
  return lowerBoundFilters.every(
    (f) => (f.operator === ">=" && f.value <= 0) || (f.operator === ">" && f.value < 0),
  );
}
...
export async function applyCommentFilters({
  ...
  const commentContentFilters = filterState.filter(
    (f): f is CommentContentFilter => f.type === "string" && f.column === "commentContent",
  );
  ...
  if (commentCountFilters.length > 0) {
    ...
    if (filterRangeIncludesZero(numberFilters)) {
      ...
      const upperBoundFilter = numberFilters.reduce<(typeof numberFilters)[number] | undefined>(
        (tightest, filter) => {
          const isUpperBound =
