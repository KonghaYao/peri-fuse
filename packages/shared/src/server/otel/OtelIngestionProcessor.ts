...
import { ForbiddenError, ObservationLevel, ObservationType, ObservationTypeDomain } from "../../";
import { env } from "../../env";
import {
...
import { flattenJsonToPathArrays, isValidDateString } from "./utils";
...
interface CreateTraceEventParams {
  ...
  resourceAttributeMetadata: Record<string, any>;
  spanAttributeMetadata: Record<string, any>;
  scopeSpan: any;
...
export class OtelIngestionProcessor {
  ...
  async publishToOtelIngestionQueue(resourceSpans: ResourceSpan[]) {
    ...
    if (isLiteMode()) {
      const { processEventBatchLite } = await import("../ingestion/processEventBatchLite.js");
    ...
    // Upload to S3
    await getS3EventStorageClient(env.LANGFUSE_S3_EVENT_UPLOAD_BUCKET!).uploadJson(
      fileKey,
          ...
          .flatMap((resourceSpan) => {
            ...
            for (const scopeSpan of resourceSpan?.scopeSpans ?? []) {
              ...
              for (const span of scopeSpan?.spans ?? []) {
                ...
                const spanMetadata = this.extractMetadata(spanAttributes, "observation", span);
                const traceMetadata = this.extractMetadata(spanAttributes, "trace", span);
                ...
                const serviceVersion = resourceAttributes?.["service.version"] as
                  | string
                  | undefined;
                const telemetrySdkLanguage = resourceAttributes?.["telemetry.sdk.language"] as
                  | string
                  | undefined;
                const telemetrySdkName = resourceAttributes?.["telemetry.sdk.name"] as
                  | string
                  | undefined;
                const telemetrySdkVersion = resourceAttributes?.["telemetry.sdk.version"] as
                  | string
                  | undefined;
                ...
                recordDistribution("langfuse.ingestion.otel.event.byte_length", eventBytes, {
                  source: "otel",
                ...
                const experimentFields = this.extractExperimentFields(spanAttributes, span);
                ...
                const toolDefinitions =
                  Object.keys(normalizedTools.toolDefinitions).length > 0
                    ...
                    : undefined;
                const toolCalls =
                  normalizedTools.toolCalls.length > 0 ? normalizedTools.toolCalls : undefined;
                const toolCallNames =
                ...
                events.push({
                  ...
                  promptName: canLinkPrompt
                    ? (spanAttributes?.[LangfuseOtelSpanAttributes.OBSERVATION_PROMPT_NAME] ??
                      spanAttributes["langfuse.prompt.name"] ??
                      this.parseLangfusePromptFromAISDK(spanAttributes)?.name ??
                      null)
                    : null,
                  promptVersion: canLinkPrompt
                    ? (spanAttributes?.[LangfuseOtelSpanAttributes.OBSERVATION_PROMPT_VERSION] ??
                  ...
                  modelName: isAiSdkAgentSpan ? undefined : this.extractModelName(spanAttributes),
                  ...
                  traceName: spanAttributes?.[LangfuseOtelSpanAttributes.TRACE_NAME] ?? null,
  ...
  private processSpan(
    ...
    const spanAttributeMetadata = this.extractMetadata(attributes, "observation", span);
    ...
    const isRootSpan =
      !parentObservationId || String(attributes[LangfuseOtelSpanAttributes.AS_ROOT]) === "true";
  ...
  private extractPublic(attributes?: Record<string, unknown>): boolean | undefined {
    const value =
      attributes?.[LangfuseOtelSpanAttributes.TRACE_PUBLIC] ?? attributes?.["langfuse.public"];
  ...
  private createObservationEvent(params: CreateObservationEventParams): IngestionEventType {
    ...
    const observation = {
      ...
      level:
        ...
        (span.status?.code === 2
          ? ObservationLevel.ERROR
          : scopeSpan?.scope?.name === "livekit-agents" && LIVEKIT_DEBUG_SPAN_NAMES.has(span.name)
            ? ObservationLevel.DEBUG
            : ObservationLevel.DEFAULT),
      statusMessage:
      ...
      usageDetails: isAiSdkAgentSpan
        ? {}
        : this.extractUsageDetails(attributes, instrumentationScopeName, observationContext),
      costDetails: isAiSdkAgentSpan ? {} : this.extractCostDetails(attributes, observationContext),
  ...
  private convertOtelIntValue(intValue: any): number | undefined {
    ...
    if (intValue && typeof intValue === "object") {
      ...
      if (typeof intValue.high === "number") {
        return intValue.high * 2 ** 32 + intValue.low;
      }
    }
    return undefined;
  }
  ...
  private extractInputAndOutput(params: {
    ...
    if (instrumentationScopeName === "genkit-tracer") {
      const genkitInput =
        this.parseJsonPayload(attributes["genkit:input"]) ?? attributes["genkit:input"];
      const genkitOutput =
        this.parseJsonPayload(attributes["genkit:output"]) ?? attributes["genkit:output"];
    ...
    if (instrumentationScopeName === "ai") {
      ...
      if ("ai.response.text" in attributes && "ai.response.toolCalls" in attributes) {
        ...
        output =
          "ai.response.text" in attributes && attributes["ai.response.text"]
      ...
      const genAiInputOutput = this.extractOpenTelemetryGenAiInputAndOutput(attributes);
    ...
    for (const event of operationDetailsEvents) {
      ...
      const genAiInputOutput = this.extractOpenTelemetryGenAiInputAndOutput(eventAttributes);
      if (genAiInputOutput) {
    ...
    if (inputEvents.length > 0 || outputEvents.length > 0) {
      ...
      return {
        ...
        output:
          processedOutput && processedOutput.length === 1 ? processedOutput[0] : processedOutput,
    ...
    output = events.find(
      (event: Record<string, unknown>) => event.name === "gen_ai.content.completion",
    )?.attributes;
    ...
    // LiveKit
    input =
      attributes["lk.input_text"] ?? attributes["lk.user_transcript"] ?? attributes["lk.chat_ctx"];
   ...
   */
  private prependSystemInstructions(input: unknown, systemInstructions: unknown): unknown {
    try {
        ...
        if (Array.isArray(parsed)) {
          content = parsed
            .map((p: Record<string, unknown>) =>
              typeof p === "object" && p?.content ? String(p.content) : String(p),
            )
            .join("\n");
        } else {
  ...
  private extractMetadata(
    ...
    if (primaryValue !== undefined && primaryValue !== null && !primaryValue) {
      this.recordMetadataDropped(typeof primaryValue === "string" ? "parse_failure" : "primitive", {
        domain,
        attributeKey: metadataKeyPrefix,
        dropScope,
      });
    }
    ...
    const langfuseMetadata = this.extractPrefixedMetadataAttributes({
      attributes,
      prefixes: [metadataKeyPrefix, "langfuse.metadata", "ai.telemetry.metadata"],
    ...
    if (attributes[LangfuseOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS]) {
      try {
        return this.sanitizeModelParams(
          JSON.parse(attributes[LangfuseOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS] as string),
        );
      } catch {
        // Fallthrough
      }
  ...
  private isAiSdkAgentOperation(attributes: Record<string, unknown>): boolean {
    ...
    return (
      typeof operationName === "string" && ["invoke_agent", "agent_step"].includes(operationName)
    );
  }
      ...
      try {
        ...
        if (providerMetadata) {
          ...
          if ("anthropic" in parsed && "usage" in parsed.anthropic) {
            const anthropicMetadata = parsed.anthropic.usage as Record<string, unknown>;
            ...
            if (
              ...
              const cacheCreation = anthropicMetadata.cache_creation as Record<string, number>;

              if (typeof cacheCreation.ephemeral_5m_input_tokens === "number") {
                usageDetails.input_cache_creation_5m ??= cacheCreation.ephemeral_5m_input_tokens;
              }
              if (typeof cacheCreation.ephemeral_1h_input_tokens === "number") {
                usageDetails.input_cache_creation_1h ??= cacheCreation.ephemeral_1h_input_tokens;
              }
  ...
  private extractGenericGenAiUsageDetails(
    ...
    const rawUsageDetails = usageDetails.reduce((acc: Record<string, number>, key) => {
    ...
    const inputTokens =
      rawUsageDetails.prompt_tokens ?? rawUsageDetails.input_tokens ?? rawUsageDetails.prompt;
    const outputTokens =
      rawUsageDetails.completion_tokens ??
      rawUsageDetails.output_tokens ??
      rawUsageDetails.completion;
    const totalTokens = rawUsageDetails.total_tokens ?? rawUsageDetails.total;
    ...
    const normalizedUsageDetails = Object.entries(rawUsageDetails).reduce(
      (acc: Record<string, number>, [key, value]) => {
        ...
        const normalizedKey = key.startsWith("details.") ? key.replace("details.", "") : key;
  ...
  private extractCompletionStartTime(
    ...
    try {
      const value = attributes[LangfuseOtelSpanAttributes.OBSERVATION_COMPLETION_START_TIME] as any;
  ...
  private extractTags(attributes: Record<string, unknown>): string[] {
    const tagsValue =
      ...
      attributes[`${LangfuseOtelSpanAttributes.OBSERVATION_METADATA}.langfuse_tags`] ||
  ...
  private recordMetadataDropped(reason: string, context: MetadataDropContext): void {
    ...
    if (this.metadataDropWarnCount < OtelIngestionProcessor.METADATA_DROP_WARN_CAP) {
  ...
  private extractExperimentFields(
    ...
    const experimentName = attributes[LangfuseOtelSpanAttributes.EXPERIMENT_NAME];
    const experimentDescription = attributes[LangfuseOtelSpanAttributes.EXPERIMENT_DESCRIPTION];
    const experimentDatasetId = attributes[LangfuseOtelSpanAttributes.EXPERIMENT_DATASET_ID];
    const experimentItemId = attributes[LangfuseOtelSpanAttributes.EXPERIMENT_ITEM_ID];
    ...
    const experimentItemVersion = attributes[LangfuseOtelSpanAttributes.EXPERIMENT_ITEM_VERSION];
    ...
    return {
      ...
      experimentItemVersion: this.parseExperimentItemVersion(experimentItemVersion),
  ...
  private parseExperimentItemVersion(value: unknown): string | undefined {
    ...
    logger.warn("OTEL invalid experiment item version, dropping. Expected timestamp.");
    return undefined;
  }
  ...
  private parseId(data: any): string {
    ...
    return typeof actualData === "string" ? actualData : Buffer.from(actualData).toString("hex");
  }
   ...
   */
  private getConversionFailureLogContext(resourceSpans: ResourceSpan[]): Record<string, unknown> {
    return {
      projectId: this.projectId,
      sdkName: this.sdkName,
      sdkVersion: this.sdkVersion,
  ...
  private getInstrumentationScopes(resourceSpans: ResourceSpan[], limit = 10): string[] {
    try {
      if (!Array.isArray(resourceSpans)) {
        return [];
      }
