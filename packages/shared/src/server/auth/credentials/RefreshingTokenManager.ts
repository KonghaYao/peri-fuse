...
export class RefreshingTokenManager {
  ...
  constructor(provider: ManagedCredentialProvider, options: RefreshingTokenManagerOptions = {}) {
    this.provider = provider;
    const ratio = options.expirationRefreshRatio ?? DEFAULT_EXPIRATION_REFRESH_RATIO;
    if (ratio <= 0 || ratio >= 1) {
      throw new RangeError(`expirationRefreshRatio must be in the range (0, 1), got ${ratio}`);
    }
  ...
  private notify(token: ManagedAccessToken): void {
    for (const listener of this.listeners) {
      try {
        ...
        logger.warn(`Managed credential refresh listener threw for ${this.provider.name}`, error);
      }
    }
  }
}
