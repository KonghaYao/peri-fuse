  ...
  return instrumentAsync(
    ...
    async (span) => {
      if (logger.isLevelEnabled("debug")) {
        logger.debug(
          formatModelMatchDebugMessage("Resolving model match", {
            ...
            model: p.model,
            localCacheEnabled: env.LANGFUSE_LOCAL_CACHE_MODEL_MATCH_ENABLED === "true",
            redisCacheEnabled: env.LANGFUSE_CACHE_MODEL_MATCH_ENABLED === "true",
          }),
        );
      }
      const localCacheKey = getRedisModelKey(p);
      const { source, value } = await modelMatchLocalCache.getOrLoad(localCacheKey, async () => {
      ...
      if (!value || value.model === null) {
        ...
        if (source === "none" && env.LANGFUSE_CACHE_MODEL_MATCH_ENABLED === "true") {
          span.setAttribute("model_cache_set", "true");
        }

        if (logger.isLevelEnabled("debug")) {
          logger.debug(
            formatModelMatchDebugMessage("Model match resolved without a model", {
              projectId: p.projectId,
              model: p.model,
              source: source ?? "none",
              pricingTierCount: 0,
...
const addModelNotFoundTokenToRedis = async (p: ModelMatchProps) => {
  try {
    const key = getRedisModelKey(p);
    await redis?.set(key, NOT_FOUND_TOKEN, "EX", env.LANGFUSE_CACHE_MODEL_MATCH_TTL_SECONDS);
  } catch (error) {
