...
import { env } from "../../env";
import { InvalidRequestError, LangfuseNotFoundError, UnauthorizedError } from "../../errors";
import type { AuthHeaderValidVerificationResultIngestion } from "../auth/types";
import { getClickhouseEntityType, type IngestionEntityTypes } from "../clickhouse/schemaUtils";
import {
  getCurrentSpan,
  instrumentAsync,
  recordDistribution,
...
import { type StorageService, StorageServiceFactory } from "../services/StorageService";
import {
  HASH_HEX_LENGTH,
...
import { createIngestionEventSchema, eventTypes, type IngestionEventType } from "./types";
...
export const processEventBatch = async (
  ...
  if (authCheck.scope.orgId) currentSpan?.setAttribute("langfuse.org.id", authCheck.scope.orgId);
  if (authCheck.scope.plan) currentSpan?.setAttribute("langfuse.org.plan", authCheck.scope.plan);
  ...
  await instrumentAsync({ name: "s3-upload-events" }, async () => {
    ...
    const results = await Promise.allSettled(
      Object.keys(sortedBatchByEventBodyId).map(async (id) => {
        ...
        return getS3StorageServiceClient(env.LANGFUSE_S3_EVENT_UPLOAD_BUCKET!).uploadJson(
          bucketPath,
          data,
        );
      }),
