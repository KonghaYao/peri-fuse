...
function addToolArguments(args: ClickhouseToolArgument[], calls: unknown[] | undefined): void {
  if (!calls) return;
    ...
    for (const choice of obj.choices) {
      if (choice && typeof choice === "object") {
        ...
        const messageToolCalls =
          parseArrayIfString(message?.tool_calls) ?? parseArrayIfString(message?.toolCalls);
  ...
  if (Array.isArray(obj.content)) {
    for (const part of obj.content) {
      if (
        part &&
        typeof part === "object" &&
        ["tool_use", "tool-call"].includes((part as Record<string, unknown>).type as string)
      ) {
...
function extractToolCallsFromMessage(
  ...
  const messageToolCalls = parseArrayIfString(msg.tool_calls) ?? parseArrayIfString(msg.toolCalls);
  addToolArguments(args, messageToolCalls);
...
function toIngestionJsonValue(value: unknown): IngestionJsonValue {
  if (value == null) return value as IngestionJsonValue;
...
export function extractToolsFromObservation(
  ...
  try {
    return extractToolsFromParsedObservation(parseIfString(input), parseIfString(output));
  } catch (error) {
