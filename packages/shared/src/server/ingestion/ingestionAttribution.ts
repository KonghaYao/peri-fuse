...
const getHeaderValue = (headers: IngestionHeaderMap | undefined, name: string): string => {
  if (!headers) return "";
...
export const classifyIngestionSdkVersion = (params: {
  ...
  const parsedVersion = parseVersionString(extractBaseIngestionSdkVersion(sdkVersion));
...
export const classifyIngestionSdkAttribution = (params: {
  ...
  const missingVersion = !sdkVersion || sdkVersion === UNKNOWN_INGESTION_SDK_VALUE;
...
export const createIngestionAttribution = (params: {
  ...
  return {
    ...
    ingestionSdkName: normalizeSdkValue(getHeaderValue(headers, "x-langfuse-sdk-name")),
    ingestionSdkVersion: normalizeSdkValue(getHeaderValue(headers, "x-langfuse-sdk-version")),
  };
};
