...
describe("ingestion attribution", () => {
  ...
  ])("classifies $sdkName@$sdkVersion SDK upgrade status", ({ sdkName, sdkVersion, expected }) => {
    expect(classifyIngestionSdkVersion({ sdkName, sdkVersion })).toEqual(expected);
  });
  ...
  ] as const)("classifies %s@%s attribution as %s", (sdkName, sdkVersion, expected) => {
    expect(classifyIngestionSdkAttribution({ sdkName, sdkVersion })).toBe(expected);
  });
});
