...
function normalizeMessage(msg: unknown): Record<string, unknown> {
  ...
  if (working.type === "function_call_output") {
    const content =
      typeof working.output === "string" ? working.output : JSON.stringify(working.output ?? "");
  ...
  const normalized = removeNullFields(working);
...
export const openAIAdapter: ProviderAdapter = {
  ...
  detect(ctx: NormalizerContext): boolean {
    ...
    if (meta && typeof meta === "object") {
      ...
      if ("scope" in meta && meta.scope !== null && typeof meta.scope === "object") {
