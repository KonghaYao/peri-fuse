 ...
 */
export function normalizeToolDefinitionForChatMl(tool: unknown): Record<string, unknown> | null {
  const parsedTool = parseIfString(tool);
  if (!isPlainRecord(parsedTool)) return null;

  const nestedFunction = isPlainRecord(parsedTool.function) ? parsedTool.function : undefined;
...
export function normalizeToolDefinitionsForChatMl(tools: unknown): Array<Record<string, unknown>> {
