 ...
 */
import { z } from "zod";
import { ChatMlArraySchema, type ChatMlMessageSchema } from "../IORepresentation/chatML/types";
import { type NormalizerContext, selectAdapter } from "./adapters";
...
export function mapToChatMl(input: unknown): ReturnType<typeof ChatMlArraySchema.safeParse> {
  const result = ChatMlArraySchema.safeParse(input);
  if (result.success) {
    return result;
  }
