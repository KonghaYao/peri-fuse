...
describe("findMediaReferences", () => {
  ...
  it("finds references in nested objects and arrays", () => {
    const value = {
      messages: [{ role: "user", content: [{ type: "image_url", image_url: imageRef }] }],
    };
  ...
  it("ignores references embedded in surrounding text", () => {
    expect(findMediaReferences({ text: `Compare ${imageRef} against this.` })).toEqual([]);
  });

  it("handles objects with a literal 'match' key", () => {
    expect(findMediaReferences({ match: imageRef })).toMatchObject([{ jsonPath: "$['match']" }]);
  });
  ...
  it("skips malformed reference strings", () => {
    expect(findMediaReferences({ broken: "@@@langfuseMedia:no-key-value@@@" })).toEqual([]);
  });
