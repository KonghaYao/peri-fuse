...
import type { JsonNested } from "./zod";
  ...
  while (stack.length > 0) {
    ...
    if (typeof input === "object" && input !== null) {
      ...
      const hasDangerousKeys =
        !isArray && Object.keys(input as object).some((key) => DANGEROUS_KEYS.has(key));
...
const JSON_NUMBER_LITERAL_PATTERN = /^-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?$/;
...
const preserveUnsafeNumbers: ReviverWithSource = (_key, value, context) =>
  typeof value === "number" && context?.source !== undefined && !isSafeNumber(context.source)
    ? context.source
    : value;
