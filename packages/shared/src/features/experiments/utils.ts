...
export const datasetItemMatchesVariable = (input: Prisma.JsonValue, variable: string) => {
  if (input === null || input === undefined || typeof input !== "object" || Array.isArray(input))
    return false;
  return Object.keys(input).includes(variable);
};

const isValidPrismaJsonObject = (input: Prisma.JsonValue): input is Prisma.JsonObject =>
  typeof input === "object" && input !== null && input !== undefined && !Array.isArray(input);

/**
 * Validate dataset item input against prompt variables.
 * Allows plain strings if exactly one variable exists.
 */
export const validateDatasetItem = (itemInput: Prisma.JsonValue, variables: string[]): boolean => {
  ...
  return variables.some((variable) => datasetItemMatchesVariable(itemInput, variable));
};
