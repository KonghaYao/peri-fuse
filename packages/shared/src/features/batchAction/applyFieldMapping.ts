...
import type { AddToDatasetMapping, FieldMappingConfig, SourceField } from "./addToDatasetTypes";
  ...
  switch (config.mode) {
    ...
    case "custom": {
      ...
      if (config.custom.type === "root") {
        ...
        const evaluated = safeEvaluate(rootConfig.sourceField, rootConfig.jsonPath, null);
      ...
      if (config.custom.type === "keyValueMap") {
        ...
        for (const entry of keyValueMapConfig.entries) {
          ...
          if (isJsonPath(entry.value)) {
            const evaluated = safeEvaluate(entry.sourceField, entry.value, entry.key);
 ...
 */
export function generateJsonPathSuggestions(data: unknown, prefix = "$"): string[] {
