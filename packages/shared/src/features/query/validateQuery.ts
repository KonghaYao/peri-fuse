...
function hasSameFieldEntityDimensionBound(filters: QueryType["filters"], field: string): boolean {
...
function validateEntityDimension(query: QueryType, version: ViewVersion): QueryValidationResult {
  const view = getViewDeclaration(query.view, version);

  const field = query.entityDimension?.field;
  const dimension = view.dimensions[field];

  if (!dimension || dimension.explodeArray || dimension.pairExpand || dimension.aggregationFunction)
    return {
      valid: false,
      reason: `Invalid entity dimension: ${field}. Entity dimensions must be scalar view dimensions.`,
    };

  if (dimension.highCardinality && !hasSameFieldEntityDimensionBound(query.filters, field)) {
    return {
      valid: false,
...
export function validateQuery(query: QueryType, version: ViewVersion): QueryValidationResult {
  ...
  const chartConfig =
    (query as unknown as { config?: QueryType["chartConfig"] }).config ?? query.chartConfig;
 ...
 */
export function toQueryChartConfig<T extends { type: string; bins?: number; row_limit?: number }>(
  widgetChartConfig: T,
